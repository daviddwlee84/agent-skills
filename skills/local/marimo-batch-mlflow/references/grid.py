...
This script samples random hyperparameter combinations and launches each one
as a separate job. By default it does a dry run so you can inspect the
combinations before committing to actual compute.
...
Tracking backend:
    Runs are tracked to MLflow. The launched job reads `MLFLOW_TRACKING_URI`
    (and optionally `MLFLOW_TRACKING_TOKEN`) from the env passed via secrets.
    If the URI is unset on the worker, MLflow falls back to a local
    `./mlruns/` directory inside the job's workdir, which is usually NOT what
    you want for a remote sweep — so `MLFLOW_TRACKING_URI` is required here.
...
Note on compute providers:
    ...
    out for Modal, RunPod, or local `subprocess.run([...])` for in-process
    sweeps. The search-space sampling and dry-run formatting are
    provider-agnostic — only `launch_run()` needs to change.
"""
...
# Hugging Face Jobs is just one compute provider — see the note above.
# Replace this import + launch_run() body with your provider of choice.
...
FIXED = {
    "mlflow_experiment": "batch-sizes",
}

# Required secrets passed to the worker. HF_TOKEN authenticates with HF Jobs;
# MLFLOW_TRACKING_URI tells the worker where to log to.
REQUIRED_SECRETS = ("HF_TOKEN", "MLFLOW_TRACKING_URI")
OPTIONAL_SECRETS = ("MLFLOW_TRACKING_TOKEN",)
...
def sample_runs(count: int, rng: random.Random) -> list[dict[str, str]]:
    ...
    while len(runs) < target:
        ...
        run.update(FIXED)
        run["mlflow_run_name"] = build_run_name(run)
        runs.append(run)
...
def print_run(index: int, total: int, params: dict[str, str]) -> None:
    ...
    print(f"[{index}/{total}] {params['mlflow_run_name']}")
...
def launch_run(
    index: int, total: int, params: dict[str, str], secrets: dict[str, str]
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--launch",
        action="store_true",
        help="Actually submit the sampled runs. Without this flag, only a "
        "dry run is printed.",
    )
