...
**`App::Music::ChordPro`**. Installing it provides two commands: **`chordpro`**
and **`a2crd`**. Authoritative:
...
```bash
a2crd input.txt                 # ChordPro to stdout
a2crd input.txt -o song.cho
chordpro --a2crd input.txt -o song.cho   # same, via the main binary
```

`--a2crd` (ASCII → crd) is **distinct** from `--generate=ChordPro` (reparse +
re-emit an existing ChordPro file). The heuristics are tunable
(`pct_chords`, `classic`, etc. — see
https://www.chordpro.org/chordpro/chordpro-configuration-a2crd/); output usually
needs a light manual pass, so **always validate and eyeball** before rendering.
