...
> `skills/local/`, `skills/owned/`, and `skills/vendor/` trees.
...
- **`skills/owned/`** — first-party skills maintained in another project, synced through `vendor.yaml` with `collection: owned`. Edit the canonical source there; this repo distributes the copy. See the [owned workflow](docs/workflows/adding-owned-skills.md).
  - [`go-cli-tui`](skills/owned/go-cli-tui/) — From [awesome-lazy-tools](https://github.com/daviddwlee84/awesome-lazy-tools); build Go CLIs, Lazygit-inspired dashboards, and guided wizards with Charm, arrow/Vim navigation, XDG preferences, and real terminal verification. ([docs](https://daviddwlee84.github.io/agent-skills/skills/go-cli-tui/))
...
Each plugin entry defines one category and explicitly lists its member skills;
anything not listed there falls under **Other** automatically. The manifest is
the source of truth for the current category set rather than a duplicated list
in this README.
...
# Exercise Claude Code + Codex native add/list/install in isolated state.
